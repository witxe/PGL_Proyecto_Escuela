...
public class Activity3Formulario extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            case R.id.action_guardar:
                intent = new Intent(Activity3Formulario.this, ActivityLista.class);
                validar();
                break;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.example.juan.proyecto.R.layout.activity_formulario);
        ...
        switch(llamada){
            ...
            case 2:
                ...
                if (alumno != null) {
                    ...
                    item_id.setText(String.valueOf(alumno.getId()));
    ...
    void validar(){
        ...
        if(TextUtils.isEmpty(nombre)) {
            txtNombre.setError(getString(com.example.juan.proyecto.R.string.error_obligatorio));
            txtNombre.requestFocus();
            return;
        }
        if(TextUtils.isEmpty(email)) {
            txtEmail.setError(getString(com.example.juan.proyecto.R.string.error_obligatorio));
            txtEmail.requestFocus();
            return;
        }
        if (!isValidEmail(email)){
            txtEmail.setError(getString(com.example.juan.proyecto.R.string.error_email));
            txtEmail.requestFocus();
            return;
        }
        if(TextUtils.isEmpty(telefono)) {
            txtTelefono.setError(getString(com.example.juan.proyecto.R.string.error_obligatorio));
            txtTelefono.requestFocus();
            return;
        }
        if (txtTelefono.length() != 9) {
            txtTelefono.setError(getString(com.example.juan.proyecto.R.string.error_telefono));
            txtTelefono.requestFocus();
            return;
        }
        if (idRadio == -1) {
            Toast.makeText(getApplicationContext(),getString(com.example.juan.proyecto.R.string.error_sexo), Toast.LENGTH_SHORT).show();
            return;
        }
        if(TextUtils.isEmpty(edadS)) {
            txtEdad.setError(getString(com.example.juan.proyecto.R.string.error_obligatorio));
            txtEdad.requestFocus();
            return;
        }
        int edad = Integer.parseInt(edadS);
        if (edad<5 || edad>99) {
            txtEdad.setError(getString(com.example.juan.proyecto.R.string.error_edad));
            txtEdad.requestFocus();
            return;
        }
